...
export async function POST(req: Request) {
  try {
    ...
    const { lastInsertRowid } = db
      .prepare('INSERT INTO receivables (user_id, name, description, amount, month_created) VALUES (?, ?, ?, ?, ?)')
...
export async function PATCH(req: Request) {
  try {
    ...
    const row = db.prepare('SELECT * FROM receivables WHERE id = ? AND user_id = ?').get(id, userId) as {
    ...
    db.transaction(() => {
      db.prepare('UPDATE receivables SET amount_paid = ?, paid = ?, month_paid = ? WHERE id = ? AND user_id = ?')
        .run(newPaid, fullyPaid ? 1 : 0, fullyPaid ? month : null, id, userId);
      db.prepare('INSERT INTO income_entries (user_id, description, amount, month, source) VALUES (?, ?, ?, ?, ?)')
...
export async function DELETE(req: Request) {
  try {
    ...
    getDb().prepare('DELETE FROM receivables WHERE id = ? AND user_id = ?').run(id, userId);
    return NextResponse.json({ ok: true });
  } catch (err) {
    if (err instanceof Response) return err;
    return NextResponse.json({ error: 'Server error' }, { status: 500 });
