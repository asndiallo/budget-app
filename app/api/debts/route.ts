...
export async function POST(req: Request) {
  try {
    const { userId } = getRequestUser(req);
    const db = getDb();
    const { label, lender, balance, monthly_payment, interest_rate } = await req.json();
    const result = db
      .prepare(
        'INSERT INTO debts (user_id, label, lender, balance, monthly_payment, interest_rate) VALUES (?, ?, ?, ?, ?, ?)',
      )
      .run(userId, label, lender ?? '', balance ?? 0, monthly_payment ?? 0, interest_rate ?? 0);
...
export async function PATCH(req: Request) {
  try {
    const { userId } = getRequestUser(req);
    const db = getDb();
    const { id, label, lender, balance, monthly_payment, interest_rate } = await req.json();
    db.prepare(
    ...
    ).run(label ?? null, lender ?? null, balance ?? null, monthly_payment ?? null, interest_rate ?? null, id, userId);
