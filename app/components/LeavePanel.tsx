...
function computeFromStart(
  ...
  if (
    startYear > lastYear ||
    (startYear === lastYear && startMonth > lastMonth)
  ) {
...
function monthsBetween(fromISO: string, toDate: Date): number {
  ...
  return (
    (toDate.getFullYear() - d.getFullYear()) * 12 +
...
export default function LeavePanel({ basePay, joinedAt }: Props) {
  ...
  async function recordLeave() {
    ...
    try {
      ...
      setEvents((prev) =>
        [...prev, newEvent].sort((a, b) => a.taken_at.localeCompare(b.taken_at)),
      );
      setLeaveDate('');
      setLeaveDays('');
      setLeaveNote('');
  ...
  const daysToOct1 = Math.ceil(
    (oct1.getTime() - today.getTime()) / (1000 * 60 * 60 * 24),
  );
  ...
  // Anchor staleness
  const anchorAgeMonths = anchor
    ? monthsBetween(anchor.les_period, today)
    : null;
  ...
  const visibleEvents = anchor
    ? events.filter(e => e.taken_at > anchor.les_period)
    : events;
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => { setShowLesForm(v => !v); setShowLeaveForm(false); }}
            className={`text-[11px] transition-colors ${
              showLesForm
                ? 'text-text-3'
                : 'text-[#00d98a] hover:text-[#00d98a]/80'
            }`}
          >
          ...
          <button
            onClick={() => { setShowLeaveForm(v => !v); setShowLesForm(false); }}
      ...
      {/* LES anchor status */}
      {anchor ? (
        <div className={`flex items-center gap-2 mb-3 text-[11px] px-3 py-2 rounded-lg border ${
          isStale
      ...
      {/* LES import form */}
      {showLesForm && (
        <div className="mb-4 p-3 rounded-xl border border-[#00d98a]/30 bg-[#00d98a]/5 space-y-2">
          <p className="text-[11px] font-semibold text-[#00d98a] uppercase tracking-wider">
          ...
          <div className="flex gap-2 flex-wrap items-end">
            <div className="space-y-0.5">
              <p className="text-[10px] text-text-4">LES period end date</p>
              <input
                type="date"
                value={lesPeriod}
                onChange={e => setLesPeriod(e.target.value)}
                className="text-sm bg-bg border border-border rounded-lg px-2 py-1.5 text-text focus:outline-none focus:border-[#00d98a]/60 transition-colors"
              />
            </div>
            <div className="space-y-0.5">
              <p className="text-[10px] text-text-4">EOM leave balance (days)</p>
              <input
                type="number"
                min={0}
                step={0.5}
                ...
                className="w-28 font-mono text-sm bg-bg border border-border rounded-lg px-2 py-1.5 text-text focus:outline-none focus:border-[#00d98a]/60 transition-colors"
              />
            </div>
            <button
              onClick={importLes}
              disabled={savingLes || !lesPeriod || !lesBalance}
              className="text-sm px-3 py-1.5 rounded-lg bg-[#00d98a] hover:bg-[#00d98a]/80 text-black font-medium transition-colors disabled:opacity-40"
            >
          ...
          {anchor && (
            <p className="text-[10px] text-text-4">
              Previous anchor: {fmtDate(anchor.les_period)} · {anchor.balance_days} days —
              will be replaced. Leave events before the new LES date will be ignored in calculations.
            </p>
          )}
        </div>
      )}
      ...
      {/* Record leave form */}
      {showLeaveForm && (
        <div className="mb-4 p-3 rounded-xl border border-border bg-surface space-y-2">
          ...
          <div className="flex gap-2 flex-wrap">
            <input
              ...
              className="text-sm bg-bg border border-border rounded-lg px-2 py-1.5 text-text focus:outline-none focus:border-blue-600 transition-colors"
            />
            <input
              type="number"
              min={0.5}
              ...
              placeholder="Days"
              className="w-20 font-mono text-sm bg-bg border border-border rounded-lg px-2 py-1.5 text-text focus:outline-none focus:border-blue-600 transition-colors"
            />
            <input
              value={leaveNote}
              ...
              className="flex-1 min-w-32 text-sm bg-bg border border-border rounded-lg px-2 py-1.5 text-text focus:outline-none focus:border-blue-600 transition-colors"
            />
            <button
              onClick={recordLeave}
              disabled={savingLeave || !leaveDate || !leaveDays}
              className="text-sm px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white transition-colors disabled:opacity-40"
            >
              {savingLeave ? '…' : 'Save'}
            </button>
          </div>
          {anchor && (
            <p className="text-[10px] text-text-4">
              Only leave taken after {fmtDate(anchor.les_period)} affects the balance —
              earlier dates are covered by your LES import.
            </p>
          )}
        </div>
      )}
      ...
      {basePay > 0 && (
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="rounded-xl border border-border bg-surface-raised/20 p-3">
            <p className="text-[11px] text-text-3 mb-1">Terminal leave value</p>
          ...
          <div className="rounded-xl border border-border bg-surface-raised/20 p-3">
            <p className="text-[11px] text-text-3 mb-1">Sell-back at separation</p>
            <p className="text-lg font-mono font-semibold text-text">
      ...
      {willExceedCap && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 mb-3">
          ...
          <p className="text-[11px] text-amber-500/70 mt-0.5">
            Projected balance: {projectedBalance.toFixed(1)} days in {daysToOct1} days
            — exceeds the {NORMAL_CAP}-day carry-over cap. Use leave before Oct 1.
          </p>
        </div>
      )}
      ...
      {visibleEvents.length > 0 && (
        <div className="mb-3">
          <p className="text-[11px] font-semibold text-text-4 uppercase tracking-wider mb-2">
          ...
          <div className="space-y-0.5">
            {visibleEvents.map((e) => (
              <div
                key={e.id}
                className="flex items-center gap-2 text-xs py-1.5 border-b border-border-dim"
              >
                <span className="font-mono text-text-3 shrink-0">{e.taken_at}</span>
                <span className="font-mono text-[#ff4560] shrink-0">
                  −{e.days} day{e.days !== 1 ? 's' : ''}
                </span>
                {e.note && (
                  <span className="text-text-4 flex-1 truncate">{e.note}</span>
                )}
                <button
                  onClick={() => deleteEvent(e.id)}
                  className="ml-auto text-text-4 hover:text-[#ff4560] transition-colors shrink-0"
                >
                  ✕
                </button>
              </div>
      ...
      {supersededCount > 0 && (
        <p className="text-[10px] text-text-4 mb-3">
          {supersededCount} leave record{supersededCount !== 1 ? 's' : ''} before the LES date
          {' '}are superseded by the LES import and not counted.
        </p>
      )}

      {/* Quick reference */}
      <div className="text-[11px] text-text-4 space-y-0.5 pt-2 border-t border-border-dim">
        <p>· Accrual: {ACCRUAL_RATE} days/month · carry-over cap: {NORMAL_CAP} days (Oct 1)</p>
